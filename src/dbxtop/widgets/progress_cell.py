...
def render_progress(completed: int, total: int, failed: int = 0, width: int = 20) -> str:
    ...
    # Clamp to prevent bar wider than width when completed > total
    if ok_chars + fail_chars > width:
        fail_chars = max(0, width - ok_chars)
    empty_chars = max(0, width - ok_chars - fail_chars)
